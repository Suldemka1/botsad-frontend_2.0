...
export default function Home({ content, news, page, pageSize, pageCount, total, links, sliderLinks }) {
    ...
    for (let number = 1; number <= pageCount; number++) {
        items.push(
            <button key={number}
                className="bg-green-800 dark:bg-gray-500 border rounded px-3 py-2 text-white cursor-pointer"
                onClick={() => {
                        ...
                        links.map((item) =>
                            <div key={item.id} className="flex justify-center items-center">
                                <Link href={item.url} key={item.id}>
                                    <a>
                                        <Image alt='some' key={item.id} src={process.env.APIpath + item.image.url}
                                               width={200}
                                               height={200}/>
                                    </a>
                                </Link>
